...
export const reduceFraction = ([ numerator, denominator ]: Fraction): Fraction => {
    if ([ numerator, denominator ].some((num) => typeof num !== "number" || isNaN(num))) {
...
const greatestCommonDivisor = (numerator: number, denominator: number): number => {
    if (denominator === 0) {
        return numerator;
    }
...
export const sumFraction = (a: Fraction, b: Fraction): Fraction => {
    ...
    if ([ numA, denA, numB, denB ].some((num) => typeof num !== "number" || isNaN(num))) {
        throw new Error(invalidFraction);
    }
