...
export type NaturalNoteNameCirclePositionMap = {
  readonly [key in NaturalName]: NaturalCirclePosition;
};
...
export const pitchDefinitionFromMIDINoteNumber = ({
  midiNoteNumber,
  assumedAccidental = 0,
}: MIDINoteNumberWithAssumedAccidental): PitchDefinition => {
  const natural = midiNoteNumber - assumedAccidental;
  const naturalNormalized = normalizedModulo(natural, 12);
  const naturalName = (Object.keys(
    naturalNoteNameSemitonePosition
  ) as NaturalName[]).find(
    (name) => naturalNoteNameSemitonePosition[name] === naturalNormalized
  );
  ...
  const circlePosition =
    naturalNoteNameCirclePosition[naturalName] + assumedAccidental * 7;
...
export const pitchDefinitionFromNameString = (
  name: string
): PitchDefinition => {
  ...
  if (accidentalStr) {
    accidentals -=
      2 * (accidentalStr.match(doubleFlatSymbolsRegex) || []).length;
    accidentals -= (accidentalStr.match(flatSymbolsRegex) || []).length;
    accidentals += (accidentalStr.match(sharpSymbolsRegex) || []).length;
    accidentals +=
      2 * (accidentalStr.match(doubleSharpSymbolsRegex) || []).length;
  }
  const octave = Number(octaveStr);

  const naturalCirclePos =
    naturalNoteNameCirclePosition[naturalName as NaturalName];
