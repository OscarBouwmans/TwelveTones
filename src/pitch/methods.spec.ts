...
describe("PitchMethods", () => {
    ...
    describe("transposedBy()", () => {
        testPitches.forEach((startPitch) => {
            it(`${startPitch.naturalName()} ${startPitch.accidentals()} ${startPitch.octave}`, () => {
                expect(startPitch.transposedBy(
                    createInterval({ perfect: "unison" }),
                ).isEqualTo(startPitch)).toBeTrue();
                expect(startPitch.transposedBy(
                    createInterval({ augmented: "unison" }),
                ).isEqualTo(startPitch)).toBeFalse();
                expect(startPitch.transposedBy(
                    createInterval({ augmented: "unison" }),
                ).natural().isEqualTo(startPitch.natural())).toBeTrue();
                expect(startPitch.transposedBy(
                    createInterval({ augmented: "unison" }),
                ).isEnharmonicEquivalentOf(startPitch)).toBeFalse();
                ...
                ).isEnharmonicEquivalentOf(startPitch)).toBeTrue();
                ...
                expect(startPitch.transposedBy(
                    ...
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                ).isEqualTo(startPitch, false)).toBeFalse();
                expect(startPitch.transposedBy(
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                ).isEqualTo(startPitch, true)).toBeFalse();
                expect(startPitch.transposedBy(
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                ).isEnharmonicEquivalentOf(startPitch, true)).toBeTrue();
                expect(startPitch.transposedBy(
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                    createInterval({ minor: "third" }),
                ...
                expect(startPitch.transposedBy(
                    ...
                    createInterval({ perfect: "octave", octaveShift: -2, direction: -1 }),
