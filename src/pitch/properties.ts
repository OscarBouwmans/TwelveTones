...
export const pitchProperties: PitchProperties = {
  ...
  name() {
    ...
    return `${this.naturalName()}${"♯".repeat(sharps)}${"♭".repeat(flats)}${
      this.octave
    }`;
  },
  naturalName() {
    ...
    return Object.keys(naturalNoteNameCirclePosition).find((key) => {
      return (
        naturalNoteNameCirclePosition[key as NaturalName] === naturalCirclePos
      );
    }) as NaturalName;
  },
  naturalNameIndex() {
  ...
  midiNoteNumber() {
    const semitonePosition =
      naturalNoteNameSemitonePosition[this.naturalName()];
    return 60 + semitonePosition + 12 * (this.octave - 4) + this.accidentals();
  },
  semitonePosition() {
    const naturalSemitonePosition =
      naturalNoteNameSemitonePosition[this.naturalName()];
