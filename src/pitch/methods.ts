...
export interface PitchMethods {
     ...
     */
    readonly isEqualTo: (this: Pitch, other: Pitch, ignoreOctave?: boolean) => boolean;
...
export const pitchMethods: PitchMethods = {
    isEqualTo(other: Pitch, ignoreOctave: boolean = false) {
        return (this.circlePosition === other.circlePosition) && (ignoreOctave || (this.octave === other.octave));
    },
    ...
    transposedBy(...intervals: Interval[]) {
        if (intervals.length === 0) {
            return createPitch({ circlePosition: this.circlePosition, octave: this.octave });
        }
        ...
        return createPitch({
            circlePosition: this.circlePosition + (interval.direction * interval.circleShift),
